            ...
            <div class="col-md-12">
                <div class="row new-investment-user">
                    ...
                    <div class="col-sm-4 text-center">
                        <div class="panel panel-green panel-pricing">
                            <div class="panel-heading">
                                <h3 style="font-size: 28px;"><b>{{ $essential_plan->name }}</b></h3>
                            </div>
                            <div style="font-size: 18px;padding: 18px;" class="panel-body text-center">
                                <p><strong>{{ $essential_plan->min_units }} unit - {{ $essential_plan->max_units }} units</strong></p>
                            </div>
                            <ul style='font-size: 15px;' class="list-group text-center bold">
                                <li class="list-group-item"><i class="fa fa-check"></i> for {{ $essential_plan->time }}  months </li>
                                <li class="list-group-item"><i class="fa fa-check"></i> {{ $essential_plan->price }} per unit</li>
                                @foreach ($essential_plan->essentials as $value)
                                
                                <li class="list-group-item"><i class="fa fa-check"></i> {{ $value->roi }}<i class="fa fa-percent"></i>  roi for {{$value->payout_duration}}</li>
                            ...
                            <div class="panel-footer" style="overflow: hidden">
                                ...
                                <div class="col-sm-12">
                                    <button type="submit" class="btn btn-primary bold uppercase btn-block btn-icon icon-left plan_id radious-zero" value="{{$essential_plan->id}}" data-toggle="modal" data-target="#invest_review_modal2">
                                            <i class="fa fa-send"></i> Invest Under This Package
                                        </button>
                                    <form method="POST" action="{{ route('investment-post') }}" class="form-inline">
                                        {{ csrf_field() }}
                                        <input type="hidden" name="id" value="{{ $essential_plan->id }}">
        ...
        <div class="col-md-4">
            <div class="col-sm-12 text-center">
                <div class="panel panel-default panel-pricing">
                    ...
                    <ul style="font-size: 15px;" class="list-group text-center bold">
                        @foreach ($essential_plan->essentials as $item)
                        <li class="list-group-item" style="padding: 18px 0px;">
                        <i class="fa fa-check"></i> <span id="percent"></span> <i class="fa fa-percent"> {{$item->roi}} <span id="time"></span>{{$item->payout_duration}}</i> 
                        </li>  
                        @endforeach
                    ...
                    <input type="hidden" value="{{$essential_plan->time}}" id="time12">
                        {{-- <li class="list-group-item" style="padding: 18px 0px;">
                            <i class="fa fa-check"></i> For <span id="tim"></span> Months 
                        </li> --}}
                        {{-- <li class="list-group-item" style="padding: 18px 0px;">
                             ...
                             <li class="list-group-item">
                                ...
                                <div class="col-md-7 text-left">
                                    <select name="essential" id="percent_roi" class="form-control">
                                        @foreach ($essential_plan->essentials as $item)
                                        <option value="{{$item->roi}}">{{$item->payout_duration}}</option>
                                        @endforeach
